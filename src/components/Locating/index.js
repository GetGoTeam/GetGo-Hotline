...
export default function Locating(props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const origin = await microkernel.locateAddress(originAddress, "GoongPlugin");
        if (origin) setOriginCoord(origin);
        const destination = await microkernel.locateAddress(destinationAddress, "GoongPlugin");
